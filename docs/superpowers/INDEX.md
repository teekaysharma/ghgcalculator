...
| Hooks (build-time guardrails) — [intent](intents/2026-09-11-build-hooks-intent.md) · [spec](specs/2026-09-11-build-hooks-design.md) | Spec approved | Design done 2026-09-11: four-category methodology (record-of-a-past-event, secret/no-clean-undo, named-boundary-crossing, would-it-impede-legitimate-work) applied to protected paths, a credential scan driven live off `.env.example`, and a Prettier config/reformat plan. Plan not yet written. Includes the full one-time repo-wide reformat that triggers "Next priorities" item 2 above. |
