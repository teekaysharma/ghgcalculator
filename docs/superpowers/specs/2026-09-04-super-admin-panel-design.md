...
- **A super-admin can also demote another super-admin back to a normal account**, with a required note explaining why — the note is stored in the audit log alongside the action, so a demotion always carries a recorded justification. A super-admin can never demote *themselves* (the panel rejects it with `400`): this is the one hard guard rail in the feature, and it exists specifically so the panel itself can never be used to drop the platform to zero super-admins — even though the migration's seed is self-healing (it re-fires whenever the count is truly zero), a lone super-admin demoting themselves would still leave the platform with zero super-admins until someone noticed and manually re-ran the migration, rather than staying continuously usable.
...
- `users.is_super_admin boolean not null default false`. Excluded from `insertUserSchema`'s pick list — self-serve registration can never set it. The migration seeds it to `true` for the user whose email is the project owner's, gated on a zero-count check (`SELECT COUNT(*) FROM users WHERE is_super_admin = true` is `0`) rather than "column was just added this run". This makes the seed self-healing: it fires as soon as that account's row exists and the script is (re-)run, and it also recovers if the platform is ever reset to zero super-admins. It's still safe against re-promoting a deliberately demoted account, because demoting always requires a *different* super-admin to act (self-demote is rejected), so at least one super-admin always remains after any single demote through the app. This remains the only way to create the *first* super-admin; every subsequent one comes from the in-panel promote action below.
- `admin_action_log` (new table): `id`, `actor_user_id` (FK → `users.id`, no cascade — the actor is always an authenticated super-admin session), `action` (`"verify" | "delete" | "promote" | "demote"`, plain text like `memberships.role`), `target_user_id` (int, **not** an FK — a delete's whole point is removing that row, so a hard FK would fight the delete rather than survive it), `target_email` (denormalized snapshot, keeps the row legible after the target is gone), `note` (nullable text — required by the demote route, unused by the other three), `created_at`.
...
- `demoteFromSuperAdmin(userId): Promise<void>` — the inverse `UPDATE ... = false`.
- `logAdminAction(entry): Promise<AdminActionLog>` — thin insert, used standalone by the verify, promote, and demote routes; `entry.note` is optional (only demote ever sends it).
- `listAdminActionLog(): Promise<AdminActionLogEntry[]>` — the most recent 200 entries (a simple cap, not full pagination — this is a lower-traffic view than the account list), joined to `users` for the actor's email so the log reads as "who did what to whom, and why for a demotion," ordered newest-first.
- `verifyUserEmail(userId)` (existing) is reused unmodified for the admin verify action.

Audit-log placement is intentionally asymmetric: verify/promote/demote each call their respective state-changing operation, then a separate `logAdminAction` in its own try/catch, so a logging hiccup never turns a successful action into a 500 (baking an actor-aware log write into the shared `verifyUserEmail` itself would wrongly log ordinary self-service verifications too, since that method is also called from the public `/api/auth/verify-email` flow). Delete's log write is folded into its `db.batch` because delete is irreversible; losing the "who did this" record to a transient failure is a worse outcome there.

**`server/routes.ts`** — six new routes, `requireAuth, requireSuperAdmin`, no `requireOrg`, no rate limiter (authenticated + role-gated already, no unauthenticated attack surface to throttle — matches the existing `requireAuth`-only reference-data routes, which also carry none):
...
| `POST /api/admin/users/:id/demote` `{ note }` | `200 { user }` | `400` missing/empty `note`; `400` if `id` is the caller's own id (self-demote is always rejected — see the Decisions section above for why); `404` unknown id; `400` if target isn't currently a super-admin |
...
A second, read-only "Recent activity" table on the same page lists the most recent `admin_action_log` entries (actor, action, target, note, when) — this is the audit log the "no read endpoint" decision above was revised to include, and the note column is where a demotion's stated reason becomes visible to every other super-admin.
...
Covers: non-admin gets 403 on all routes; admin's list includes its own row; search actually filters; verify flips DB state and logs an action; delete removes a pending user and still leaves an audit row behind (`target_user_id` survives as a non-FK reference); deleting a verified user is rejected with 409 and changes nothing; promoting a verified user flips `is_super_admin` and logs it; promoting an unverified user is rejected with 400; demoting a super-admin (with a note) flips `is_super_admin` back and logs the note; demoting without a note, demoting a non-admin, and self-demote are all rejected with 400 and change nothing; the action-log endpoint returns the entries just created, including the demote note; unknown ids 404 across the relevant routes.
...
- Any way to demote yourself, or for a second super-admin to be created automatically if the platform is ever down to zero (that scenario is prevented by construction, not handled after the fact — see the self-demote guard rail above).
