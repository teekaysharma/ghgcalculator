...
**The verification link carries the email address alongside the token** — `/verify-email?token=...&email=user@example.com` — not just the token. This is what makes the failure state below possible to handle gracefully instead of ambiguously: the token alone can't tell the frontend who to help once it's invalid, but the visitor's own inbox already shows them which address the email was sent to, so passing it through costs nothing and turns a dead end into a one-click recovery.

New endpoint `POST /api/auth/resend-verification-email { email }`:
- Regardless of whether the email exists or is already verified, returns the same generic `200` response (`{ message: "If that email has a pending registration, we've sent a new link." }`) — avoids confirming account existence to an unauthenticated caller.
...
New frontend page `client/src/pages/VerifyEmail.tsx`, routed at `/verify-email`: reads `token` and `email` from the URL on mount, immediately calls `POST /api/auth/verify-email`, and renders one of three states:
- **Verifying** — brief spinner while the request is in flight.
- **Success** — confirmation + link to `/login`.
- **Failed** (covers both "token not found" and "token expired" — see above for why these can't be distinguished once cleanup has run, and why that's fine here): the page **immediately and automatically** calls the resend endpoint with the email from the URL — no button, no extra click. It can genuinely still be a live account at this point: token expiry (24h) and the cleanup sweep (once daily) aren't the same moment, so an expired-but-not-yet-cleaned-up row can exist for **up to ~48 hours** after registration before it's actually deleted. Automatically resending covers exactly that (common) window with zero friction.

  But the resend response can't say whether it actually found an account to resend for — same generic response either way, for the same enumeration-protection reason it's needed on the Login page's "unverified" resend button, which calls this same endpoint. If the row's already gone (past that ~48h mark), the auto-resend is a silent no-op, and an earlier version of this design left the visitor waiting on an email that was never going to arrive, with the real fix (register again) tucked into small, easy-to-miss text. The server can't resolve that ambiguity honestly — but the *visitor* can, immediately, because they know something the server doesn't: how long ago they actually signed up. So both outcomes are shown with equal weight, and the copy hands them the one fact that lets them self-diagnose which applies:

  *"We've sent a fresh verification link to **user@example.com** — check your inbox (and spam folder)."*
  *"Registered more than 2 days ago? That one's already been cleaned up — create a new account instead."* — with a real, properly-weighted **"Create a new account"** button right there, not a small trailing link.

  2 days is a real, defensible number, not a guess: 24h token expiry + up to 24h more until the next daily cleanup sweep = 48h worst case before a row is actually gone.

This replaces an earlier version of this design that (a) showed a resend button and a "register again" link as two co-equal, unexplained choices, and (b) after fixing that, still buried "register again" as a barely-visible fallback under an auto-resend message that can silently do nothing. Giving the visitor the actual timing rule, not just a vague hedge, is what makes the two paths resolvable by someone who has no way to see the database.

The email itself links to this page — a real app route — rather than pointing directly at the API endpoint, because plain `GET` links get pre-fetched by corporate email-security scanners in many environments, which would silently burn a one-time token before the real recipient ever clicks it. Routing through a page that only fires the mutation from client-side JS on deliberate load avoids that.
