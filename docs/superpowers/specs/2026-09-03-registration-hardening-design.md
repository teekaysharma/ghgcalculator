...
- **Failed** (covers both "token not found" and "token expired" — see above for why these can't be distinguished once cleanup has run, and why that's fine here): the page **immediately and automatically** calls the resend endpoint with the email from the URL — no button, no extra click — then shows *"That link has expired. If you have a pending registration for **user@example.com**, we've just sent a fresh one — check your inbox."* The conditional phrasing ("if you have a pending registration") is honest rather than overclaiming: this page genuinely cannot tell whether the account still exists or was already cleaned up, and the resend endpoint's response can't reveal that either (see above) — but the sentence reads as reassuring and complete regardless of which case is true, and the visitor gets a real new link within the (large majority of) cases where the account is still there, with zero clicks. Below that, smaller and clearly secondary: *"Already verified, or didn't mean to sign up? [Log in](/login) or [Create a new account](/register)."*

This replaces an earlier version of this design that showed a resend button and a "register again" link as two co-equal, ambiguous choices — confusing, since only one of them is ever actually the right one and the page has no way to say which. Leading with one automatic, honestly-worded action, and demoting the alternative to a small fallback line, is both more helpful and less likely to frustrate someone who's already had one broken link.
