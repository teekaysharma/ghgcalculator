...
2. **Read the unit from `io.unit`, check it equals `"M.EUR"` via an explicit `raise`** (not a bare `assert`, which `python -O` strips entirely) — fail loudly rather than silently compute wrong numbers if a future EXIOBASE release changes this.
3. `io.calc_all()`.
4. Extract all 25 genuine GHG-bearing rows from `io.air_emissions.M` (CO2/CH4/N2O combustion and non-combustion — fugitive fossil-fuel-extraction CH4, cement/lime process CO2, agriculture, waste — plus SF6/HFC/PFC), not just the original 4 combustion rows — a whole-branch review found the original 4-row set captured only ~70% of true GHG mass, worst in oil & gas/cement/agriculture. Air-quality pollutants also tracked in the same extension (PM10/PM2.5, NOx, SOx, heavy metals, VOCs, etc.) are excluded — they are not GHGs. Biogenic CO2 stays excluded (2 rows: combustion and waste), matching this app's existing `isBiogenic` convention; biogenic CH4/N2O are included at their own GWP-100 value, same as the original set already did for `CH4_bio`. See `scripts/exiobase/build_factors.py`'s `GWP_KEY` comment for the full row-by-row classification and reasoning (including the fossil/non-fossil CH4 split and why `HFC`/`PFC` are summed at weight 1, not looked up — EXIOBASE reports both already in `kg CO2-eq`, not a raw mass of one named gas).
5. Apply AR6 GWP-100 weights **read from this app's own `gwpValues` table**, not re-typed — exported to `scripts/exiobase/gwp_weights.json` by `scripts/manual-migration-011.mjs` (which also seeds the one additional gas the fuller stressor set needs, SF6, into `gwpValues`) so the Python script never hardcodes a weight.
...
7. For each region-sector cell, check `io.x` (total output): if it is zero or NaN, the multiplier is mathematically undefined (0/0) — emit `null` for `kgCo2ePerEur` rather than a computed `0.0`, which would be indistinguishable from a genuine low-carbon factor. This affects ~25.7% of cells overall and ~31% of WM (the UAE's own EXIOBASE region) cells.
8. Write one JSON file per table type: `region, regionLabel, sector, tableType, kgCo2ePerEur (nullable), factorYear, exiobaseVersion, sourceUrl, computedAt`.
9. A hand-written idempotent migration (`scripts/manual-migration-010.mjs`, following the established numbering) loads both JSON files plus the EPA NAICS-6 CSV plus a seeded `countryExiobaseRegions` table into the three new tables — matching this project's confirmed-necessary pattern (`drizzle-kit push` is broken for multi-table/FK diffs on this Neon database, per `MIGRATIONS.md`). `exiobase_factors` rows carry a `notes` marker distinguishing a corrected reseed from the old data, so re-running this migration after a `build_factors.py` fix truncates and reseeds rather than skipping or duplicating.

Runs once now; re-run only when a newer EXIOBASE release is deliberately adopted (own decision, own re-validation — not an automatic upgrade), or when a computation fix like the one described above is made.

**Outlier values (updated post-fix, 2026-09-02):** the real EXIOBASE data contains a handful of numerically extreme cells (e.g. GB / Manufacture of medical, precision and optical instruments) from near-zero domestic output in specific region-sector cells — a known MRIO artifact, not a computation bug. Some of what previously drove this is now resolved: cells where output is *exactly* zero are stored as `NULL` (see step 7 above), not an extreme or a zero value, which removes the most severe zero-division cases from the extreme-value tail. Cells with output that is merely very small (not zero) can still produce a large-but-defined multiplier — these remain real, faithfully computed data, not something to filter/alter in this pipeline. The fuller 25-row GHG stressor set (step 4) also shifts which sectors show up as outliers, since non-combustion fugitive/process emissions are no longer omitted. Whoever specs the future mapping/calculation phase should still add a plausibility ceiling or outlier flag before any of these values reach a user-facing report.
