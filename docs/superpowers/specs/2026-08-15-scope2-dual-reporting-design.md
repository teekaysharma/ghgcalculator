...
- **Most countries (blended):** one row per country, `sector = 'all'`, `gasType = 'CO2e'` (a new pseudo-gas value). `buildComponent` in `client/src/lib/ipccGasBundle.ts` treats `gasType === 'CO2e'` as already-blended: `co2ePerUnit = nativeFactor` directly, with **no further GWP multiplication applied by this tool** (re-multiplying an already-blended external figure by our own AR6 table would double-count the GWP the source already applied — a worse error than the one this guards against). `gwpGasKeyFor` needs a one-line addition for this case (skip the internal `gwp_values` table lookup for `'CO2e'` rows).

  **Verified during design (not assumed):** IEA's electricity factors are genuinely multi-gas — they include CH4 and N2O converted to CO2e, not CO2-only — so treating a row as one blended component doesn't silently drop a gas the way a completeness check would flag. What the design got wrong on the first pass: it specified `GWP = 1` with no disclosure, as if no GWP were involved at all. It was — IEA applied one internally to produce the blended figure — and GHG Protocol's Corporate Standard requires the GWP source/version/horizon behind *every* figure in the inventory to be disclosed explicitly, not just figures this tool computes itself. **Correction: these rows must still populate `gwpVersion`/`gwpSource` on their `gasBreakdown` component, sourced from the source dataset's own disclosed methodology** (confirmed during actual data sourcing — IEA's Emissions Factors documentation states its GWP-100 basis; cite it exactly, don't assume it matches this tool's AR6 standard just because the values look close). If a source's documentation doesn't state its GWP basis clearly enough to cite with confidence, the row must be flagged as "GWP basis as originally applied by the source, not independently re-derived" rather than silently implying AR6 consistency it hasn't been confirmed to have. `activityType` = `"Grid Electricity"` / `"District Cooling"`.

  US eGRID rows are unaffected by this correction and were already designed correctly: they use eGRID's *raw separate* CO2/CH4/N2O rates, not eGRID's own pre-blended CO2e output (which has used SAR, then AR4, then AR5 GWPs at different points — never AR6), and re-multiply through this tool's own AR6 `gwp_values` table exactly like Stationary Combustion does.
...
1. **Ember global country-level grid electricity factors** (215 countries, updated twice monthly) — Ember's Yearly/Monthly Electricity Data, released under CC BY 4.0 (commercial use and redistribution permitted with attribution, confirmed directly on ember-energy.org). **Correction, 2026-09-02**: this spec originally named IEA's Emissions Factors database as the primary source. Verified directly against IEA's own product page before implementation: IEA's data costs €995+ per license and its standard terms explicitly prohibit "third-party clients or carbon footprint verification work without a separate commercial agreement" — exactly this product's use case. A free alternative, CaDI (Carbon Data Intelligence), was also checked and rejected: its Terms & Conditions (CC BY-NC-ND 4.0) explicitly exclude "consultancy work that is delivered to clients." Ember was verified clean and is the corrected source. Attribution requirement (CC BY 4.0) must be carried into the app's own source citation for these rows, same as every other seeded factor.
